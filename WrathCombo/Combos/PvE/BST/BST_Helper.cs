...
internal partial class BST
{
    ...
    // porting-note(api13) B1: Beastmaster is game 7.4+ content and cannot be played on TC 7.20,
    // and the rows these read (XBMPet.Pet, Pet.AllowedPetMirage) plus the XBMManager instance do
    // not exist on this client's sheets / CS 6966. Stubbed inert rather than guessed; the whole BST
    // module is unreachable here, so nothing observable changes. Restore verbatim from upstream
    // once TC reaches 7.4.
    public static unsafe bool CurrentPetIsBMPet => false;
    ...
    // module is unreachable here, so nothing observable changes. Restore verbatim from upstream
    // once TC reaches 7.4.
    public static unsafe bool PetUnlocked(uint petId) => false;
    ...
    // module is unreachable here, so nothing observable changes. Restore verbatim from upstream
    // once TC reaches 7.4.
    public static uint PetIdToModel(uint petId) => 0;
    ...
    // module is unreachable here, so nothing observable changes. Restore verbatim from upstream
    // once TC reaches 7.4.
    public static uint ModelToPetId(uint modelId, byte baseval) => 0;
