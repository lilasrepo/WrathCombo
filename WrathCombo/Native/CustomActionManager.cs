...
public sealed unsafe class CustomActionManager : IDisposable
{
                               ...
                               IFramework framework)
    {
        ...
        // [TC api12] The "Custom Actions" feature (inject combos into hotbar slots) hooks several game
        // functions via signature scan. Some sigs (e.g. SigGetActionTransient) are game-7.5 and are absent
        // in the TC game-7.1 binary -> ScanText throws KeyNotFoundException, which (unguarded) crashed the
        // whole plugin load. B1-stub: guard setup so a missing sig disables only Custom Actions; core
        // auto-rotation still loads. The TC baseline never shipped this feature.
        try
        {
        ...
        {
            Svc.Log.Warning(ex, "[WrathCombo] TODO(api12): Custom Actions hook/sig setup failed on game 7.1; feature disabled (plugin still loads).");
            _getActionRowHook?.Dispose();
            _isSlotUsableHook?.Dispose();
            _loadIconHook?.Dispose();
            _updateTooltipHook?.Dispose();
            _updateNameHook?.Dispose();
            _getActionRowHook = null;
            _isSlotUsableHook = null;
            _loadIconHook = null;
            _updateTooltipHook = null;
            _updateNameHook = null;
        }
    }
    ...
    public void Dispose()
    {
        ...
        {
            detailAgent->Hide();
            // porting-note(api13): upstream's FFXIVClientStructs types this field as the
            // superset DetailKind (which has None = 0); api13's CS types it as the narrower
            // ActionKind, whose members start at Action = 28 and include no None. The two
            // enums share numeric values. `default` is 0 -- same result as upstream's
            // DetailKind.None -- and avoids naming ActionKind, which is ambiguous here
            // between the FFXIVClientStructs and Dalamud namespaces this file imports.
            detailAgent->ActionKind = default;
            detailAgent->ActionId = 0;
            detailAgent->OriginalId = 0;
            detailAgent->AdjustedId = 0;
        }

        // porting-note(api13): null-conditional is TC-side and must survive. The five
        // hooks are set up inside a try/catch because their game-7.5 sigs are absent on
        // TC's game version -- on a failed scan they stay null, and upstream's bare
        // .Dispose() would NRE here and take the whole plugin's teardown with it.
        _getActionRowHook?.Dispose();
        _isSlotUsableHook?.Dispose();
        _loadIconHook?.Dispose();
        _updateTooltipHook?.Dispose();
