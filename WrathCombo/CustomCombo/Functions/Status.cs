...
internal abstract partial class CustomComboFunctions
{
    ...
    // porting-note(api13): upstream's IStatus does not exist on this Dalamud; keep TC's
    // concrete Dalamud.Game.ClientState.Statuses.Status. The ushort->uint widening IS
    // upstream's and must be taken -- HasStatusEffect(uint) below (merged cleanly) calls
    // straight into this, so keeping ushort would not compile.
